...
import com.spurtreetech.sttarter.lib.helper.Connections;
import com.spurtreetech.sttarter.lib.helper.STTCallbackHandler;
...
public class MessagingService extends Service
{
    ...
    private static final String MQTT_BROKER = STTKeys.SERVER_URL; // Broker URL or IP Address
    private static final int MQTT_PORT = Integer.valueOf(STTKeys.PORT); // Broker Port
    ...
    public static void actionStart(Context ctx) {
        Intent i = new Intent(ctx,MessagingService.class);
    ...
    private synchronized void connect() {
        String url = "tcp://" + MQTT_BROKER + ":" + MQTT_PORT;

        String clientId = this.getSharedPreferences(STTKeys.STTARTER_PREFERENCES,0).getString(STTKeys.APP_KEY,"")+mDeviceId;

        Log.i(DEBUG_TAG,"Connecting with URL: " + url);
        if(mDataStore != null) {
            Log.i(DEBUG_TAG,"Connecting with DataStore");
            mClient = Connections.getInstance(this).createClient(this, url, clientId);
        } else {
        ...
        mConnHandler.post(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mClient.setCallback(new STTCallbackHandler(MessagingService.this, clientHandle, AppController.getInstance().getNotificationHelperListener()));
                    ...
                    if (!TextUtils.isEmpty(getSharedPreferences(STTKeys.STTARTER_PREFERENCES,0).getString(STTKeys.SUBSCRIBED_TOPICS,""))){

                        String[] topicsss = getSharedPreferences(STTKeys.STTARTER_PREFERENCES,0).getString(STTKeys.SUBSCRIBED_TOPICS,"").split(",");

                        int[] qos = new int[topicsss.length];
                        for(int i=0; i<topicsss.length; i++) {
                            qos[i] = 1;
                        }

                        mClient.subscribe(topicsss,qos);
                    }
